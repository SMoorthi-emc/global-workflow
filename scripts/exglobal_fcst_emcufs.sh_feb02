...
EOF
      ...
      FHMAX=$((FHMAX+6))              # If doing IAU, change forecast hours
...
ncores_per_node:            ${cores_per_node:-24}
...
if [ $QUILTING = .true. ] ; then
  ...
  while [ $fhr -le $FHMAX ] ; do
    FH3=$(printf %03i $fhr)
    atmo=$CDUMP.t${cyc}z.atmf$FH3.$output_file
    sfco=$CDUMP.t${cyc}z.sfcf$FH3.$output_file
    logo=$CDUMP.t${cyc}z.logf$FH3.$output_file
    rm $memdir/$atmo ; $memdir/$sfco ; $memdir/$logo
    eval $NLN $memdir/$atmo $atmo
    eval $NLN $memdir/$sfco $sfco
    eval $NLN $memdir/$logo logf$FH3
    FHINC=$FHOUT
...
#if [ $QUILTING = .true. ] ; then
#  for type in atm sfc ; do
#    $NCP $DATA/*$type*f*.tile*.nc $ROTDIR/
##   $NCP $DATA/*$type*f*.nemsio   $ROTDIR/
#  done
#else
#  sdate=$(echo $CDATE | cut -c1-8)
#  if [ $REMAP_GRID = latlon ] ; then
#    export file_list=${file_list:-'atmos_4xdaily atmos_static grid_spec nggps2d nggps3d fv3_history fv3_history2d'}
#    export PREFILE=${PREFILE:-${CDATE}0000.}
#  else
#    export file_list=${file_list:-'fv3_history fv3_history2d'}
#  fi
#  export PREFILE=${PREFILE:-""}
#  for type in $file_list ; do
#    for n in $(seq 1 $ntiles) ; do
##    $NCP $DATA/${CDATE}0000.$type.tile${n}.nc $ROTDIR/.
##    $NCP $DATA/$sdate.$type.tile$n.nc $ROTDIR/${CDATE}0000.$type.tile$n.nc
#     $NCP $DATA/$sdate.$type.tile$n.nc $ROTDIR/${PREFILE}$type.tile$n.nc
#    done
#  done
#fi
...
# OCN_RESTDIR=${OCN_RESTDIR:-$ROTDIR/gfs.$PDY/$cyc/OCN_RESTART}
# mkdir -p OCN_RESTDIR
# cd $DATA/MOM6_RESTART
# $NCP MOM.res*.nc *mom6*.nc $OCN_RESTDIR/

#  ICE_RESTDIR=${ICE_RESTDIR:-$ROTDIR/gfs.$PDY/$cyc/ICE_RESTART}
#  mkdir -p $ICE_RESTDIR
#  cd $DATA/restart
# ls -ltr *
#  $NCP * $ICE_RESTDIR/
